import { AbstractEntity } from '../common/abstract.entity';
import { Column, DataType, DeletedAt, Table } from 'sequelize-typescript';
import { UseDto } from '../decorators/use-dto.decorator';
import { DailyReportActivitiesDto } from '../modules/master-data/dtos/master-data.dto';

@Table({ modelName: 'daily_report_activities', freezeTableName: true })
@UseDto(DailyReportActivitiesDto)
export default class DailyReportActivitiesEntity extends AbstractEntity<DailyReportActivitiesDto> {
  @Column({ type: DataType.STRING })
  name: string;

  @Column({ type: DataType.DOUBLE(8, 4), defaultValue: 1 })
  order: number;

  @DeletedAt
  deletedAt: Date;
}
